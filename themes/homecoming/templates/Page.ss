...
<html lang="en">
    ...
    <body class="$ClassName">
        ...
        <div
            class="hero-wrapper"
            style="background-image: url($HeaderPhoto.FocusFill(1500, 450).URL)"
        ></div>
        <% else %>
        <div
            class="hero-wrapper"
            style="background-image: url({$ThemeDir}/images/homecoming_bg.jpg)"
        ></div>
        <% end_if %>
        ...
        <% include UpcomingEvents %> <% include Sponsors %> <% include Footer %>
